...
const Tooltip = ({ children, text }) => {
    ...
    return (
        <div class="relative inline-block" onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)}>
            {children}
            {showTooltip && (
                <motion.div
...
const AppModals = ({
    ...
    showStructuredDataIssuesModal, setShowStructuredDataIssuesModal, structuredDataIssuesModalData,
    showLocalSeoIssuesModal, setShowLocalSeoIssuesModal, localSeoIssuesModalData,
    ...
    showA11yIssuesModal, setShowA11yIssuesModal, a11yIssuesModalData,
}) => {
                ...
                <div className="flex space-x-2 border-b border-gray-200 mb-4">
                    {filters.map(filter => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold capitalize transition-colors ${sitemapFilter === filter ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-blue-500'}`}
                        >
                            {filter}
                        </button>
                    ))}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <thead className="bg-gray-50 sticky top-0">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">HTTP Status</th>
                            ...
                            {filteredUrls.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <span className={
                                            item.http_status >= 400 ? 'text-red-600' :
                                            item.http_status >= 300 ? 'text-orange-500' : 'text-green-600'
                                        }>
                                            {item.http_status}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex flex-col space-y-1">
                                            {item.is_disallowed && <span className="px-2 py-1 text-xs font-semibold text-yellow-800 bg-yellow-100 rounded-full">Disallowed by robots.txt</span>}
                                            {item.is_noindexed && <span className="px-2 py-1 text-xs font-semibold text-red-800 bg-red-100 rounded-full">Meta Noindex</span>}
                                        </div>
                                    </td>
                                </tr>
                            ))}
                     ...
                     {filteredUrls.length === 0 && <p className="text-center p-8 text-gray-500">No URLs match the selected filter.</p>}
                </div>
            </div>
        );
    };
    ...
    const renderA11yIssuesModalContent = (data) => {
        return (
            <div className="space-y-4">
                <p className={`font-bold text-lg ${data.status === 'good' ? 'text-green-600' : 'text-red-600'}`}>
    ...
    return (
        <>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {data.map((link, index) => (
                                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        <td className="px-4 py-3 whitespace-normal text-sm text-red-700">{link.statusCode}</td>
                                        <td className="px-4 py-3 whitespace-normal text-sm text-gray-500">{link.anchorText}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            {renderModal(
                showStructuredDataIssuesModal && structuredDataIssuesModalData?.issues?.length > 0,
                "Structured Data Issues",
                structuredDataIssuesModalData,
                (data) => (
                    <ul className="list-disc list-inside text-gray-700 p-4">
                        {data.issues.map((issue, index) => (
                            <li key={index} className="mb-2">
            ...
            {renderModal(
                showLocalSeoIssuesModal && localSeoIssuesModalData?.issues?.length > 0,
                "Local SEO Issues",
                localSeoIssuesModalData,
                (data) => (
                    <ul className="list-disc list-inside text-gray-700 p-4">
                        {data.issues.map((issue, index) => (
                            <li key={index} className="mb-2">
...
const Report = () => {
    ...
    useEffect(() => {
        ...
        const fetchReport = async () => {
            try {
                ...
                if (!response.ok) {
                    const errorText = await response.text(); // Read as text for error
                    let errorMsg = `Failed to fetch report: ${response.status} ${response.statusText} - ${errorText.substring(0, 100)}...`;
                    throw new Error(errorMsg);
                }

                const fullReportData = await response.json();
                console.log("Full Report Data from Backend:", fullReportData);
    ...
    const categoryBreakdownData = Object.keys(groupedSections).map(categoryName => {
        const issueCount = getCategoryIssueCount(categoryName, groupedSections, backendData);
    ...
    const MainScorecard = () => {
        ...
        const totalIssueCount = Object.values(groupedSections).reduce((acc, sections) => {
            return acc + sections.reduce((count, section) => {
                if (section.status === 'bad') return count + 1;
                if (section.status === 'warning') return count + 1;
                return count;
            }, 0);
        }, 0);
        ...
        return (
            <motion.div
                className="bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-xl flex flex-col items-center space-y-8 border border-gray-100"
                ...
                <div className="relative w-40 h-40 flex-shrink-0">
                    {isNaN(overallScore) ? (
                        ...
                        <>
                            ...
                            />
                            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                                <span className="text-5xl font-extrabold text-gray-900">{overallScore}%</span>
                            ...
                            return (
                                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    <div className="flex items-center space-x-3">
                                        <span className="p-2 bg-white rounded-full shadow-sm border border-gray-200">
                                            {getCategoryIcon(categoryName)}
                                        </span>
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <span className={`text-sm font-semibold ${categoryStatus === 'good' ? 'text-green-600' : categoryStatus === 'warning' ? 'text-yellow-600' : 'text-red-600'}`}>
    ...
    <AnimatePresence mode="wait">
        {groupedSections?.[categoryName]?.map((section, index) => (
            <ReportCard
                ...
                competitorWordCountData={section.id === 'content-quality-analysis' ? backendData.competitor_word_count_audit : null}
...
                    <button onClick={handleDownload}>Download Full Report</button>
                </div>
            </main>
...
                structuredDataIssuesModalData={structuredDataIssuesModalData}
                showLocalSeoIssuesModal={showLocalSeoIssuesModal}
                setShowLocalSeoIssuesModal={setShowLocalSeoIssuesModal}
                localSeoIssuesModalData={localSeoIssuesModalData}
...
                a11yIssuesModalData={a11yIssuesModalData}
            />
