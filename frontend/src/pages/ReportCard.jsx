...
import {
    MessageSquare, CheckCircle, AlertTriangle, BookOpen, Hash, TrendingUp, X, FileCode, Link as LinkIcon,
...
const ReportCard = ({
    ...
    // Updated Chart data to use Tailwind dark mode colors
    ...
    const keywordChartOptions = {
        ...
        plugins: {
            ...
            title: {
                ...
                color: 'currentColor', // Use currentColor to inherit color from parent
            },
        },
        scales: {
            y: {
                ...
                grid: {
                    color: 'rgba(107, 114, 128, 0.2)', // Gray-500 with opacity for dark mode
                }
            },
            x: {
                ticks: {
    ...
    return (
        <motion.div
            className={`w-full relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-6 rounded-xl shadow-md flex flex-col text-left border border-slate-200 dark:border-gray-700 transition-all duration-300 ease-in-out hover:shadow-lg`}
        ...
        >
            ...
            <div className="flex items-center mb-2">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white ml-2 leading-tight">
                    {title}
                </h3>
            </div>

            <p className={`text-sm text-slate-600 dark:text-gray-300 mb-4 flex-shrink-0`}>
            ...
            <AnimatePresence>
                {isExpanded && (
                    <motion.div
                        ...
                        className="text-slate-600 dark:text-gray-300 mb-4"
                        style={{ overflow: 'visible' }}
                    >
                            {title === 'SERP & Content Gap' && serpAnalysisData && (
    <div className="space-y-6 py-2 text-sm">
        {serpAnalysisData.status === 'completed' ? (
            <>
                {/* Content Gap */}
                <div className="bg-slate-50 dark:bg-gray-700/50 p-4 rounded-lg">
                    <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Word Count vs. Competitors</p>
                    <div className="flex items-baseline justify-between mt-2">
                        <div>
                            <p className="text-2xl font-bold text-slate-800 dark:text-white">{serpAnalysisData.contentGap?.userWordCount}</p>
                        ...
                        <div>
                            <p className="text-2xl font-bold text-indigo-500">{serpAnalysisData.contentGap?.competitorAverageWordCount}</p>
                ...
                {/* Common Topics */}
                {serpAnalysisData.contentGap?.commonCompetitorHeadings?.length > 0 && (
                    <div>
                        <h5 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-2">Common Competitor Topics</h5>
                        <div className="flex flex-wrap gap-2">
                            {serpAnalysisData.contentGap.commonCompetitorHeadings.map((term, i) => (
                ...
                {/* People Also Ask */}
                {serpAnalysisData.peopleAlsoAsk && serpAnalysisData.peopleAlsoAsk.length > 0 && (
                    <div>
                        ...
                        <ul className="space-y-2">
                            {serpAnalysisData.peopleAlsoAsk.slice(0, 4).map((question, i) => (
                                <li key={i} className="p-3 bg-slate-50 dark:bg-gray-700/50 rounded-lg text-slate-700 dark:text-slate-300 border-l-4 border-teal-400">
                                    {question}
                                </li>
                            ))}
                        </ul>
                ...
                {/* Competitors Table */}
                {serpAnalysisData.competitors && serpAnalysisData.competitors.length > 0 && (
                    <div>
                        <h4 className="font-semibold text-slate-700 dark:text-slate-200 mb-2">Top Competitors Analyzed</h4>
                        <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-gray-700">
                            <table className="min-w-full divide-y divide-slate-200 dark:divide-gray-700">
                                ...
                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-slate-200 dark:divide-gray-700">
                                    {serpAnalysisData.competitors.map((comp, i) => (
                                        <tr key={i}>
                                            <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-200" title={comp.title}>
            ...
            <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded-lg">
                <p className="flex items-center text-orange-700 dark:text-orange-300 font-semibold text-sm">
                    <AlertTriangle className="w-5 h-5 text-orange-500 mr-2 flex-shrink-0" />
                    <span>SERP analysis was not completed. Reason: {serpAnalysisData.reason || 'An unknown error occurred.'}</span>
                </p>
            </div>
        )}
    </div>
                        ...
                        {title === 'Title Optimization' && metadataLengthData && (
                            <div className="space-y-2 py-2">
                                <p className="flex items-center text-sm">
                                    <Info className="w-4 h-4 mr-1 text-slate-500 dark:text-gray-400" />
                        ...
                        {title === 'Content Quality Analysis' && contentAnalysisData && (
                            <div className="space-y-4 py-2">
                                <div className="bg-slate-100 dark:bg-gray-700 p-4 rounded-lg text-center flex flex-col md:flex-row md:justify-around md:items-center">
                                    <div className="flex-1 mb-2 md:mb-0">
                                        <p className="text-xs font-medium text-slate-700 dark:text-gray-300 flex items-center justify-center md:justify-start">
                                            <Hash className="w-4 h-4 mr-1 text-slate-500 dark:text-gray-400" />
                                            <span className="font-bold text-slate-900 dark:text-white">Word Count:</span>
                                        </p>
                                        <p className="text-xl font-extrabold text-indigo-600 mt-1">
                                            {contentAnalysisData.total_word_count || 'N/A'}
                                        </p>
                                    </div>
                                    <div className="flex-1 border-t md:border-t-0 md:border-l border-slate-200 dark:border-gray-600 pt-2 md:pt-0 md:pl-4">
                                        <p className="text-xs font-medium text-slate-700 dark:text-gray-300 flex items-center justify-center md:justify-start">
                                            <BookOpen className="w-4 h-4 mr-1 text-slate-500 dark:text-gray-400" />
                                            <span className="font-bold text-slate-900 dark:text-white">Readability:</span>
                                        </p>
                                    ...
                                    <div className="flex-1 border-t md:border-t-0 md:border-l border-slate-200 dark:border-gray-600 pt-2 md:pt-0 md:pl-4">
                                        <p className="text-xs font-medium text-slate-700 dark:text-gray-300 flex items-center justify-center md:justify-start">
                                            <Layers className="w-4 h-4 mr-1 text-slate-500 dark:text-gray-400" />
                                            <span className="font-bold text-slate-900 dark:text-white">Uniqueness:</span>
                                        </p>
                                ...
                                {/* {competitorWordCountData?.competitor_average > 0 && (
                                    <div className="mt-4 p-4 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 rounded-lg">
                                        <p className="font-semibold text-sm text-indigo-800 dark:text-indigo-200 mb-2">Word Count vs. Competitors:</p>
                                        <div className="flex justify-between items-center text-xs">
                                            <p className="flex flex-col text-center">
                                                <span className="font-bold text-indigo-600 text-lg">{competitorWordCountData.target_word_count}</span>
                                                <span className="text-gray-600 dark:text-gray-400">Your Page</span>
                                            </p>
                                            <p className="flex flex-col text-center">
                                            ...
                                            <p className="flex flex-col text-center">
                                                ...
                                                <span className="text-gray-600 dark:text-gray-400">Ideal Range</span>
                                            </p>
                                        </div>
                                        {competitorWordCountData.issues?.length > 0 && (
                                            <div className="mt-3">
                                                {competitorWordCountData.issues.map((issue, i) => (
                                                    <p key={i} className={`text-sm flex items-center ${issue.startsWith('✅') ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>
                                ...
                                {keywordChartData && (
    <div className="mt-4 h-64 text-gray-900 dark:text-white">
        <Bar data={keywordChartData} options={keywordChartOptions} />
                        ...
                        {title === 'Keyword Analysis' && keywordAnalysis && (
                            <div className="space-y-4 py-2">
                                {keywordAnalysis.map((kw, index) => (
                                    <div key={index} className="border-b last:border-b-0 border-slate-200 dark:border-gray-700 pb-4">
                                        <h4 className="font-bold text-base text-slate-900 dark:text-white">{kw.keyword}</h4>
                                        ...
                                        {kw.recommendations?.length > 0 && (
                                            <div className="mt-3 p-2 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                                                <h5 className="text-md font-semibold text-yellow-700 dark:text-yellow-300 mb-1">Recommendations:</h5>
                                                <ul className="list-disc list-inside text-yellow-600 dark:text-yellow-400 text-sm">
                        ...
                        {title === 'Content Structure' && headingAuditData && (
                            <div className="space-y-4 py-2">
                                <div className="grid grid-cols-3 gap-2 text-center text-sm">
                                    <div><p className="font-medium text-slate-700 dark:text-gray-300">H1</p><p className="text-xl font-bold text-indigo-600">{headingAuditData.h1_count || 0}</p></div>
                                    <div><p className="font-medium text-slate-700 dark:text-gray-300">H2</p><p className="text-xl font-bold text-indigo-600">{headingAuditData.h2_count || 0}</p></div>
                                    <div><p className="font-medium text-slate-700 dark:text-gray-300">H3</p><p className="text-xl font-bold text-indigo-600">{headingAuditData.h3_count || 0}</p></div>
                                </div>
                                {onOpenHeadingsModal && (
                                    <button
                                ...
                                {headingAuditData.issues?.length > 0 && (
                                    <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
                                        <h5 className="text-md font-semibold text-red-700 dark:text-red-300 mb-1">Issues:</h5>
                        ...
                        {title === 'Image Accessibility' && imageAnalysisData && (
                            <div className="space-y-3 py-2 text-sm">
                                <p className="font-semibold dark:text-white">Image Summary:</p>
                                <ul className="list-disc list-inside ml-4 text-slate-700 dark:text-gray-300">
                        ...
                        {title === 'Page Speed & Core Web Vitals' && pagespeedData && (
                            <div className="space-y-6 py-2 text-sm text-slate-700 dark:text-gray-300">
                                {pagespeedData.mobile && (
                                    <div className="bg-slate-50 dark:bg-gray-700 p-4 rounded-lg">
                                        <p className="flex items-center font-semibold mb-2 text-slate-800 dark:text-white">
                                        ...
                                        <div className="grid grid-cols-1 gap-2 ml-7">
                                            <p className="flex items-center">
                                                ...
                                                <Tooltip text="Largest Contentful Paint - measures loading performance">
                                                    <Info className="w-4 h-4 ml-1 text-slate-400 dark:text-gray-500" />
                                                </Tooltip>
                                            </p>
                                            <p className="flex items-center">
                                ...
                                {pagespeedData.desktop && (
                                    <div className="bg-slate-50 dark:bg-gray-700 p-4 rounded-lg mt-4">
                                        <p className="flex items-center font-semibold mb-2 text-slate-800 dark:text-white">
                        ...
                        {title === 'Link Profile' && linkAuditData && (
                            <div className="space-y-3 py-2 text-sm">
                                <p className="font-semibold text-base mb-2 dark:text-white">Link Summary:</p>
                                <div className="grid grid-cols-2 gap-4 text-center">
                                    <div className="p-3 bg-slate-100 dark:bg-gray-700 rounded-lg">
                                        <p className="text-xs text-slate-700 dark:text-gray-300">Internal Links</p>
                                        <p className="text-xl font-bold text-indigo-600">{linkAuditData.internal_links_count || 0}</p>
                                    </div>
                                    <div className="p-3 bg-slate-100 dark:bg-gray-700 rounded-lg">
                                        <p className="text-xs text-slate-700 dark:text-gray-300">External Links</p>
                                        <p className="text-xl font-bold text-indigo-600">{linkAuditData.external_links_count || 0}</p>
                                    </div>
                                </div>
                                {linkAuditData.broken_links_count > 0 && onOpenBrokenLinksModal ? (
                                    <>
                                        <p className="flex items-center text-red-600 font-semibold mt-4 dark:text-red-400">
                                    ...
                                    <p className="flex items-center text-green-600 font-semibold mt-4 dark:text-green-400">
                            ...
                            <div className="space-y-3 py-2 text-sm">
                                {structuredDataAuditData.ld_json_found ? (
                                    structuredDataAuditData.schema_types?.length > 0 ? (
                                        <>
                                            <p className="flex items-center text-green-600 font-semibold dark:text-green-400">
                                                <CheckCircle className="w-5 h-5 text-green-500 mr-2" />
                                                Schema Found:
                                            </p>
                                            <ul className="list-disc list-inside ml-7 text-slate-700 dark:text-gray-300">
                                                {structuredDataAuditData.schema_types.map((type, index) => (
                                                    <li key={index} className="text-slate-700 dark:text-gray-300">
                                                        `@type`: {type}
                                                    </li>
                                                ))}
                                            </ul>
                                        ...
                                        <p className="flex items-center text-orange-600 font-semibold dark:text-orange-400">
                                            <AlertTriangle className="w-5 h-5 text-orange-500 mr-2" />
                                            JSON-LD found, but no `@type` property detected or types were empty.
                                        </p>
                                    )
                                ) : (
                                    <p className="flex items-center text-red-600 font-semibold dark:text-red-400">
                                        <AlertTriangle className="w-5 h-5 text-red-500 mr-2" />
                        ...
                        {title === 'Local SEO' && localSeoAuditData && (
                            <div className="space-y-3 py-2 text-sm">
                                <p className="font-semibold text-base mb-2 dark:text-white">Key Local SEO Elements:</p>
                        ...
                        {title === 'Social Media Integration' && ogTwitterData && (
                            <div className="space-y-3 py-2 text-sm text-slate-700 dark:text-gray-300">
                                {(ogTwitterData.open_graph?.title?.present || ogTwitterData.twitter_cards?.title?.present) && (ogTwitterData.open_graph?.image?.present || ogTwitterData.twitter_cards?.image?.present) ? (
                                    <div className="border border-slate-300 dark:border-gray-700 rounded-lg overflow-hidden shadow-sm">
                                        <img
                                        ...
                                        />
                                        <div className="p-3 bg-slate-50 dark:bg-gray-700">
                                            <p className="text-md font-bold text-slate-900 dark:text-white mb-1">
                                    ...
                                    <p className="flex items-center text-red-600 font-semibold text-center dark:text-red-400">
                                ...
                                {ogTwitterData.issues?.length > 0 && (
                                    <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                                        <h5 className="text-md font-semibold text-yellow-700 dark:text-yellow-300 mb-1">Social Meta Issues:</h5>
                                        <ul className="list-disc list-inside text-yellow-600 dark:text-yellow-400 text-sm">
                        ...
                        {title === 'Mobile Experience' && mobileResponsivenessData && (
                            <div className="space-y-4 py-2 text-sm text-slate-700 dark:text-gray-300">
                                {mobileScreenshot ? (
                                    <>
                                        <p className="font-semibold text-base mb-2 dark:text-white">Mobile Screenshot:</p>
                                        <div className="w-full h-auto overflow-hidden rounded-lg border border-slate-200 dark:border-gray-700 shadow-inner">
                                            <img
                                    ...
                                    <div className="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-lg text-center flex items-center justify-center">
                                ...
                                <div className="mt-4">
                                    <p className="font-semibold text-base mb-2 dark:text-white">Key Mobile Audits:</p>
                                    <ul className="space-y-2">
                        ...
                        {title === 'Robots.txt Analysis' && robotsTxtData && (
                            <div className="space-y-3 py-2 text-sm">
                                <p className="flex items-center font-semibold dark:text-white">
                                ...
                                {robotsTxtData.present && (
                                    <>
                                        <p className="font-semibold mt-2 dark:text-white">Parsed Rules (User-agent: *):</p>
                                        {robotsTxtData.rules?.['*'] && (
                                            <div className="ml-4 p-2 bg-slate-100 dark:bg-gray-700 rounded-md">
                                                {robotsTxtData.rules['*'].disallow?.map((rule, j) => (
                                                    <p key={j} className="text-red-500 dark:text-red-400 font-mono">Disallow: {rule}</p>
                                                ))}
                                                {robotsTxtData.rules['*'].allow?.map((rule, k) => (
                                                    <p key={k} className="text-green-500 dark:text-green-400 font-mono">Allow: {rule}</p>
                                                ))}
                                            </div>
                                        )}
                                    </>
                                ...
                                {robotsTxtData.sitemap_path && (
                                    <p className="flex items-center mt-3 dark:text-gray-300">
                                        <LinkIcon className="w-4 h-4 mr-2 dark:text-gray-400" /> Sitemap: <a href={robotsTxtData.sitemap_path} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline ml-1 break-all dark:text-blue-400">{robotsTxtData.sitemap_path}</a>
                                    </p>
                                )}
                        ...
                        {title === 'Meta Robots Tag' && metaRobotsData && (
                            <div className="space-y-3 py-2 text-sm">
                                ...
                                {metaRobotsData.present && (
                                    <p className="ml-4 font-mono text-slate-700 dark:text-gray-300 break-all">
                                        Content: "{metaRobotsData.content || 'N/A'}"
                                    </p>
                                )}
                                <div className="flex flex-wrap gap-2 mt-2">
                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${metaRobotsData.is_noindex ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'}`}>
                                        {metaRobotsData.is_noindex ? '❌ Noindex' : '✅ Index'}
                                    </span>
                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${metaRobotsData.is_nofollow ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'}`}>
                            ...
                            <div className="space-y-3 py-2 text-sm">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {Object.keys(httpStatusAndRedirectsData).slice(0, 2).map((url, index) => {
                                        ...
                                        const statusColor = isGood ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : chain.final_status_code >= 400 ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' : 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300';
                                        ...
                                        return (
                                            <div key={index} className="p-3 bg-slate-100 dark:bg-gray-700 rounded-lg">
                                                <p className="font-semibold text-slate-900 dark:text-white truncate">{url.length > 40 ? displayUrl : url}</p>
                                                <div className="flex items-center space-x-2 mt-1">
                                                    ...
                                                    {chain.has_redirect_chain && (
                                                        <span className="text-slate-500 dark:text-gray-400 text-xs flex items-center">
                        ...
                        {title === 'Sitemap Validation' && sitemapValidationData && (
                            <div className="space-y-4 py-2 text-sm">
                                <div className="p-4 bg-slate-50 dark:bg-gray-700 rounded-lg">
                                    <p className="font-semibold text-base text-slate-800 dark:text-white mb-3">Sitemap Audit Summary</p>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                                        <div className="p-2 bg-white dark:bg-gray-800 rounded-md shadow-sm">
                                            <p className="text-xs text-slate-500 dark:text-gray-400">Total URLs</p>
                                        ...
                                        <div className="p-2 bg-white dark:bg-gray-800 rounded-md shadow-sm">
                                            <p className="text-xs text-slate-500 dark:text-gray-400">Errors</p>
                                        ...
                                        <div className="p-2 bg-white dark:bg-gray-800 rounded-md shadow-sm">
                                            <p className="text-xs text-slate-500 dark:text-gray-400">Issues</p>
                        ...
                        {title === 'Crawl Depth & Architecture' && crawlAuditData && (
                            <div className="space-y-3 py-2 text-sm">
                                {crawlAuditData.issues.some(issue => issue.startsWith('❌')) && (
                                    <div className="bg-red-50 dark:bg-red-900/30 p-3 rounded-lg flex items-center mb-4">
                                ...
                                {crawlAuditData.issues.some(issue => issue.startsWith('⚠️')) && (
                                    <div className="bg-orange-50 dark:bg-orange-900/30 p-3 rounded-lg flex items-center mb-4">
                                        <AlertTriangle className="w-5 h-5 text-orange-500 mr-2 flex-shrink-0" />
                                ...
                                <p className="font-semibold text-base mb-2 dark:text-white">Crawl Summary:</p>
                                <div className="grid grid-cols-2 gap-4 text-center">
                                    <div className="p-3 bg-slate-100 dark:bg-gray-700 rounded-lg">
                                        <p className="text-xs text-slate-700 dark:text-gray-300">Pages Crawled</p>
                                        <p className="text-xl font-bold text-indigo-600">{crawlAuditData.nodes?.length || 0}</p>
                                    </div>
                                    <div className={`p-3 rounded-lg ${crawlAuditData.orphan_pages?.length > 0 ? 'bg-red-50 dark:bg-red-900/30' : 'bg-green-50 dark:bg-green-900/30'}`}>
                                        <p className={`text-xs ${crawlAuditData.orphan_pages?.length > 0 ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'}`}>Orphan Pages</p>
                                        <p className={`text-xl font-bold ${crawlAuditData.orphan_pages?.length > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'} mt-1`}>{crawlAuditData.orphan_pages?.length || 0}</p>
                                    </div>
                                </div>
                                <div className="mt-4">
                                    {onOpenCrawlGraphModal && (
                        ...
                        {title === 'Accessibility Analysis' && a11yAuditData && (
                            <div className="space-y-4 py-2 text-slate-700 dark:text-gray-300">
                                <p className="font-semibold text-base mb-2 dark:text-white">Issues Found: {a11yAuditData.issues.length}</p>
                                {a11yAuditData.issues.length > 0 && (
                                    <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
                                        <h5 className="text-md font-semibold text-red-700 dark:text-red-300 mb-1">Accessibility Issues:</h5>
                                        <ul className="list-disc list-inside text-red-600 dark:text-red-400 text-sm">
            ...
            <button
                ...
                className="w-full mt-4 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-purple-600 dark:hover:text-purple-300 transition-colors duration-200"
            >
